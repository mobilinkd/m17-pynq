...
typedef struct {
    ...
    XAcg_top_uio_map maps[ MAX_UIO_MAPS ];
} XAcg_top_uio_info;

/***************** Variable Definitions **************************************/
static XAcg_top_uio_info uio_info;
...
static int uio_info_read_name(XAcg_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAcg_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAcg_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAcg_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAcg_top_Initialize(XAcg_top *InstancePtr, const char* InstanceName) {
	XAcg_top_uio_info *InfoPtr = &uio_info;
